...
/// ray used for ray hit tests
    struct SceneRay {
        ...
        Vector CalcPointOn(float fDistFromOrigin) const {
            return m_vOrigin + m_vDirection * fDistFromOrigin;
        }
...
/// API consumer can decide which ones to accept or filter out.
    ...
    public:
        ...
        bool HasSelectionChange() const {
            return (m_uiFlags & kIT_SelectionChange) != 0;
        }

        bool HasRotation() const {
            return (m_uiFlags & kIT_Rotation) != 0;
        }

        bool HasTranslation() const {
            return (m_uiFlags & kIT_Translation) != 0;
        }

        bool HasScale() const {
            return (m_uiFlags & kIT_Scale) != 0;
        }
        ...
        void ClearSelectionChange() {
            m_uiFlags &= ~kIT_SelectionChange;
        }

        void ClearRotation() {
            m_uiFlags &= ~kIT_Rotation;
        }

        void ClearTranslation() {
            m_uiFlags &= ~kIT_Translation;
        }
        ...
        bool IsSelected() const {
            return (m_uiFlags & kIT_IsSelected) != 0;
        }

        Matrix GetRotation() const {
            return LeapUtil::ExtractRotation(m_mtxTransform);
        }
...
/// scene manages scene objects - handles selection and movement
  ...
  LEAP_EXPORT void RayHitsDebugDrawGL() const;
        ...
        void SetFrameTransform(const Matrix &mtxFrameTransform) {
            m_mtxFrameTransform = mtxFrameTransform;
        }

        const Matrix &GetFrameTransform() const {
            return m_mtxFrameTransform;
        }

        void SetFrameScale(float fFrameScale) {
            m_fFrameScale = fFrameScale;
        }
        ...
        void SetPointableRadius(float fRadius) {
            m_fPointableRadius = fRadius;
        }

        float GetPointableRadius() const {
            return m_fPointableRadius;
        }
        ...
        void SetSelectHitTime(float fSelectHitTime) {
            m_fSelectHitTime = fSelectHitTime;
        }

        float GetSelectHitTime() const {
            return m_fSelectHitTime;
        }
        ...
        uint32_t GetNumQueuedInteractions() const {
            return m_uiNumQueuedInteractions;
        }
        ...
        bool GetUpdateContact() const {
            return (m_uiFlags & kF_UpdateContact) != 0;
        }
        ...
        bool GetUpdateRayCast() const {
            return (m_uiFlags & kF_UpdateRayCast) != 0;
        }
  ...
  LEAP_EXPORT virtual void DebugDrawGL( LeapUtilGL::eStyle drawStyle=LeapUtilGL::kStyle_Solid ) const = 0;
    ...
    return (this && (GetType() == T::ObjectType() || GetType() == SceneObject::ObjectType())) ? static_cast<T*>(this) : NULL;
        ...
        void Translate(const Vector &translation) {
            m_mtxTransform.origin += translation;
        }
        ...
        const Matrix GetRotation() const {
            return Matrix(m_mtxTransform.xBasis, m_mtxTransform.yBasis, m_mtxTransform.zBasis);
        }
        ...
        uint32_t GetNumContacts() const {
            return m_uiNumContacts;
        }

        uint32_t GetNumPointing() const {
            return m_uiNumPointing;
        }

        uint32_t GetLastNumContacts() const {
            return m_uiLastNumContacts;
        }

        bool HasInitialContact() const {
            return m_uiHasInitialContact != 0;
        }
        ...
        bool IsPendingRemoval() const {
            return m_bPendingRemoval != false;
        }
        ...
        Matrix GetWorldToObjectTransform() const {
            return LeapUtil::RigidInverse(m_mtxTransform);
        }
...
/// additional logic for your objects.
    ...
    public:
        ...
        static eSceneObjectType ObjectType() {
            static const eSceneObjectType s_type = NextObjectType();
            return s_type;
        }

        LEAP_EXPORT virtual eSceneObjectType GetType() const {
            return ObjectType();
        }
