...
/// convention is right hand rule with z- forward, y+ up, x+ right
    ...
    public:
        ...
        void SetNearClip(float fNearClip) {
            m_fNearClip = Max(0.01f, fNearClip);
        }
        ...
        float GetVerticalFOVDegrees() const {
            return m_fVerticalFOVDegrees;
        }

        void SetVerticalFOVDegrees(float fFOV) {
            m_fVerticalFOVDegrees = fFOV;
        }
...
/// Utility class for adding simple momentum to 2D or 3D UI elements.
    ...
    public:
        ...
        void setScrollSize(float fScrollSize) {
            m_fScrollSize = (fScrollSize > 0.0f ? fScrollSize : 1.0f);
        }
        ...
        void setFixedTimeStep(float fFixedTimeStep) {
            m_fFixedTimeStep = fabs(fFixedTimeStep);
        }
...
/// mutex class.  the use cases in the sample code do not require it.
    ...
    public:
        enum {
            kManagedPointerPoolSize = ManagedPointerPoolSize
        };
    ...
    private:
        ...
        struct ManagedPointerPool {
            enum {
                kPoolSize = kManagedPointerPoolSize, kDeadPointer = 0xdeadbea7
            };
            ...
            uint32_t getNumAllocated() const {
                return m_uiNumAllocated;
            }

            // number of available managed pointer entries.
            uint32_t getNumFree() const {
                return static_cast<uint32_t>(kPoolSize) - m_uiNumAllocated;
            }
    ...
    public:
        ...
        bool operator==(const SmartPointer &rhs) const {
            return m_pManagedPointer == rhs.m_pManagedPointer;
        }
